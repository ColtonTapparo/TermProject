...
//        deleteDirectoryRecursionJava6(new File(args[3]));
...
//                .master("local")
        ...
        JavaRDD<Coordinates> businesses = lines.filter(x -> {
            ...
            return new Coordinates(name, zipCode, stars);
        ...
        for (int k = 1; k <= maxK; k++) {
            ...
            sse.add(new Tuple2<>(k, elbow.reduce((a, b)-> a + b)));
        }

        JavaSparkContext jsc = JavaSparkContext.fromSparkContext(spark.sparkContext());
        jsc.parallelize(sse).saveAsTextFile(args[3]);
